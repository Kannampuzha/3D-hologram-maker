...
def makeHologram(original,scale=0.5,scaleR=4,distance=0):
    ...
    vert_y = (up.shape[1])/2
    print(up.shape[0])
    print(int(center_x-vert_x+distance))
    print(int(center_x + vert_x + distance))
    hologram[0:up.shape[0], int(center_x-vert_x+distance):int(center_x+vert_x+distance)] = up
    hologram[ hologram.shape[1]-down.shape[1]:hologram.shape[1] , int(center_x-vert_x+distance):int(center_x+vert_x+distance)] = down
...
def process_video(video):
    ...
    while(not ret):
        ...
        if ret:
            ...
            print("Error reading video. Check filename or path")
            return
...
if __name__ == '__main__' :
    #Note image should be a perfect square. Else it doesnt work
    #orig = cv2.imread("hologram.png")

    #holo = makeHologram(orig)
    process_video("h1.mp4")
    #cv2.imwrite("hologram_new.png",holo)
